...
#define GameName "Nigger"
#define ExecutableName "PixelStrike3D.exe"
#define SteamGameID "915320"
#define CheatDllName "cheat.dll"

#define LOG_INFO(x, ...) std::printf(x "\n", __VA_ARGS__);
...
void InjectModule(HANDLE processHandle, const std::wstring& modulePath)
{
	...
	{
		LOG_INFO("Could not find %s. Please, include %s in the same directory.", filename.c_str(), filename.c_str());
		system("pause");
		return;
	}
	...
	if (!resultBuffer)
	{
		LOG_INFO("VirtualAllocEx fail. (Error: %i)", GetLastError());
		InjectionProplam();
	}

	if (!WriteProcessMemory(processHandle, resultBuffer, modulePath.c_str(), modulePathSize, NULL))
	{
		LOG_INFO("WriteProcessMemory fail. (Error: %i)", GetLastError());
		InjectionProplam();
	}
	...
	{
		LOG_INFO("VirtualProtectEx fail. (Error: %i)", GetLastError());
		InjectionProplam();
	}
	...
	{
		LOG_INFO("QueueUserAPC fail. (Error: %i)", GetLastError());
		InjectionProplam();
	};
...
int main(int argc, const char* argv[])
{
	...
	std::wstring runtimePath = loaderPath + L"\\" CheatDllName;
	...
	if (pgHandle == nullptr)
	{
		LOG_INFO("OpenProcess fail. (Error: %i)", GetLastError());
		InjectionProplam();
		return 0;
	}
